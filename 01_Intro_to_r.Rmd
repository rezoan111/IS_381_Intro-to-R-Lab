...
```{r r-interface-2020, echo=FALSE, results="asis", out.width='90%'}
...
The number of girls baptized changes over time with noticeable ups and downs. Overall it does not stay flat; it varies year to year, with periods of increases and decreases.
...
```{r plot-boy-ratio-arbuthnot}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) +
  geom_line()
```

The proportion of boys stays close to 0.5 over time. It goes up and down a little, but not a lot.
...
These U.S. birth counts are much larger than Arbuthnot’s (not a similar magnitude).
...
The proportion stays fairly stable over time and is usually slightly above 0.5, so the “more boys than girls” pattern still holds in this dataset.
...
The year shown in the first row above is the year with the most total births.
